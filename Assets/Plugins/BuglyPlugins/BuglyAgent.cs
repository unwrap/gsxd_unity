...
public sealed class BuglyAgent
{
    ...
    private static string customVersion = string.Empty;
    ...
    static public void SetCustomVersion(string ver)
    {
        BuglyAgent.customVersion = ver;
    }
    ...
    public static void DebugLog (string tag, string format, params object[] args)
    {
        ...
        UnityEngine.Debug.LogFormat("[BuglyAgent] <Debug> - {0} : {1}", tag, string.Format (format, args));
    }
...
//                }
    ...
    private static void _RegisterExceptionHandler ()
    {
        try {
            ...
            SLua.Logger.LogAction = _OnLogAction;
            _isInitialized = true;
    ...
    private static void _HandleException (LogSeverity logLevel, string name, string message, string stackTrace, bool uncaught)
    {
        ...
        //º”…œ∞Ê±æ∫≈
        if(!string.IsNullOrEmpty(BuglyAgent.customVersion))
        {
            reason = "[" + BuglyAgent.customVersion + "]" + reason;
        }
