...
{
    public static bool WriteBytesFile(string filePath, byte[] bytes)
    {
        //写本地二进制文件
        string dataFilePath = Application.persistentDataPath + "/" + filePath;
        FileInfo fi = new FileInfo(dataFilePath);
        DirectoryInfo di = fi.Directory;
        if(!di.Exists)
        {
            try
            {
                DirectoryInfo newDi = Directory.CreateDirectory(di.FullName);
            }
            catch (Exception e)
            {
                return false;
